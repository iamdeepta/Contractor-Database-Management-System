...
require("config/connection.php");
	...
	if($_SESSION['OfficeID']==''){
        ...
        //$office_name_result = json_decode($office_name, true);
     	...
     	$contract_report_result = mysqli_query($conn,"SELECT tpd.ProjectName, tc.id, tc.tender_no, tc.TenderName, tc.official_estimate,tcr.ContractorName, tc.contract_amount, DATE_FORMAT(tc.DOA, '%d/%m/%Y') as doa, DATE_FORMAT(tc.NOA, '%d/%m/%Y') as noa, DATE_FORMAT(tc.DOCS, '%d/%m/%Y') as docs, DATE_FORMAT(tc.WCD, '%d/%m/%Y') as wcd, tc.bill_paid, tc.financial_progress, tc.physical_progress, tc.remarks FROM tbl_contract as tc left outer join tbl_project_division AS tpd on tpd.id = tc.ProjectID left outer join tbl_contractor AS tcr on tc.contractor_id = tcr.ID WHERE tc.OfficeID = $office_id AND tc.Remove_Status = 0 ") or die(mysqli_error($conn));

        //$contract_report_result = json_decode($contract_report, true);


         $office_name_result = mysqli_query($conn,"SELECT OfficeID, OfficeName FROM hrtoffice WHERE OfficeID = $office_id ") or die(mysqli_error($conn));
                      ...
                      <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
                        ...
                        <tbody>
                          ...
                          <tr style="background-color: #ECECEC;">
                            ...
                            <td style="font-size: 13px !important;"><?php if($contract_result['wcd']=='') echo $contract_result['wcd']; else echo 'Yet Not Completed';?></td>
                            
                            
                            <td><form action="update_contract.php?msg=<?=$contract_result['id']?>" method="post"><button type="submit" class="btn btn-success" name="update[]" style="font-size: 14px; width: 5px; height: 20px;" ><p style="color: white; margin-top: -10px;margin-left: -6px;">✐</p></button></form> <form action="" method="post"><button type="button" class="btn btn-danger" name="remove_contract[]" style="font-size: 14px; width: 5px; height: 20px;" data-toggle="modal" data-target="#exampleModalCenter<?=$contract_result['id']?>"><p style="color: white; margin-top: -10px;margin-left: -6px;">✖</p></button> </form></td>
          ...
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              ...
              <div class="modal-footer bg-whitesmoke br">
                ...
                <button type="submit" class="btn btn-primary" name="yes_contract[]">Yes</button>
              </form>
            <?php //}
              ?>
